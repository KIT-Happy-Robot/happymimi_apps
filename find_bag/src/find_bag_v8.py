...
=======
        ...
        srv = rospy.Service("find_paper_bag_v8",FindBagV8Srv,self.grasp_bag())
        rospy.loginfo("waiting find bag...")

    #人の指さした方向をもらう
    def LRCB(self, msg):
        if self.lrmsg == None and msg != "None":
            self.lrmsg = msg.data

>>>>>>> 9d84796905bce30c6ed041dcfc84451d43344fbb
    #紙バッグの座標を返してもらう
...
<<<<<<< HEAD
        ...
        try:
            ...
            rospy.sleep(0.5)
=======
        rospy.loginfo("===start find_bag===")
        self.arm_pose('carry')
        self.eef.publish(False)
        rospy.sleep(0.5)
        self.head.publish(20.0)
        rospy.sleep(0.5)
        self.get_bag_dist() #xyzの値を取得
        x1 = self.x
        z_theta = self.z * (math.pi / 12) #rθ,θ=pi/12,r=z
